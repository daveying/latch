...
const mxEdgeSegmentHandler = mxgraph.mxEdgeSegmentHandler
...
mxConstraintHandler.prototype.pointImage = new mxImage('src/assets/images/dot.gif', 10, 10);
...
function main(container: any, toolsDiv: any)
{
    ...
    var invert = false;
    const mxCellEditor = mxgraph.mxCellEditor
    ...
    if (invert)
    {
        ...
        mxCellEditor.prototype.startEditing = function (_cell, _trigger)
        {
            //@ts-ignore
    ...
    try
    {
        ...
        // type definition of insert is incorrect
        // @ts-expect-error
        v1.insert(v12);
        var v13 = v11.clone();
        v13.value = '3';
        ...
        var e6 = graph.insertEdge(parent, null, '', new mxgraph.mxCell(), new mxgraph.mxCell());
    ...
    toolsDiv.appendChild(mxUtils.button('Zoom In', function()
    {
        graph.zoomIn();
    }));

    toolsDiv.appendChild(mxUtils.button('Zoom Out', function()
    {
        graph.zoomOut();
    }));
    ...
    var listener = function(_sender: any, evt: any)
    {
    ...
    toolsDiv.appendChild(mxUtils.button('Undo', function()
    {
        undoManager.undo();
    }));

    toolsDiv.appendChild(mxUtils.button('Redo', function()
    {
        undoManager.redo();
    }));

    // Shows XML for debugging the actual model
    toolsDiv.appendChild(mxUtils.button('Delete', function()
    {
        // type definition of insert is incorrect
        // @ts-expect-error
        graph.removeCells();
    ...
    mxUtils.write(toolsDiv, 'Wire Mode');

    // Starts connections on the background in wire-mode
    var connectionHandlerIsStartEvent = graph.connectionHandler.isStartEvent;
    graph.connectionHandler.isStartEvent = function(_me)
    {
        // @ts-ignore
    ...
    graph.connectionHandler.mouseUp = function(_sender, me)
    {
        // previous filed is dynamically added to this
        // @ts-ignore
    ...
    toolsDiv.appendChild(checkbox2);
    mxUtils.write(toolsDiv, 'Grid');

    mxEvent.addListener(checkbox2, 'click', function(_evt: any)
    {
...
mxConnectionHandler.prototype.createMarker = function()
{
    ...
    // let mxCellHighlightHighlight = mxgraph.mxCellHighlight.prototype.highlight;
    // @ts-ignore
    marker.highlight.highlight = function(state: any)
    {
        if (this.state != state)
        {
            if (this.state != null)
...
const mxCylinder = mxgraph.mxCylinder
...
ResistorShape.prototype.redrawPath = function(path, _x, _y, w, h, isForeground)
{
    var dx = w / 16;
...
const mxEdgeStyle = mxgraph.mxEdgeStyle
const mxStyleRegistry = mxgraph.mxStyleRegistry
...
onMounted(() => {
  main(graphContainer.value, tools.value);
})
...
.graphContainer {
  height:100%;
  position:relative;
  background:url('src/assets/images/wires-grid.gif');
  cursor:default;
}
</style>
